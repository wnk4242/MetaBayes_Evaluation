# This Shiny generates tables for TPR, FPR for the MABF methods (two-sided test)
library(shiny)
library(dplyr)
library(tidyr)
library(stringr)
...
make_rate_table <- function(effect_size = "0",
  ...
  # --- nested structure: BF cutoff header → rep_num once → rep_n rows ---
  ...
  # return both LaTeX and HTML kables
  list(
    latex = kable(
      nested,
      format   = "latex",
      ...
      caption  = paste(
        cap,
        "across MABF methods (two-sided test) by replication sample size ($n_{\\text{rep}}$) and number of replications ($N_{\\text{rep}}$), for Bayes factor cutoffs",
