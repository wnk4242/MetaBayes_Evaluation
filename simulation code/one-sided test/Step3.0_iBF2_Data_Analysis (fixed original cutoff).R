...
########################
# To analyze data, first run Step2.0_iBF2_Data_Combination.R
source("./Step2.0_iBF2_Data_Combination.R")
...
for (pcutoff in pcutoff_values) {
    for (criterion in seq_along(criteria)) {
        ...
        iBF2_TFPNrates_0.2null <- do.call(rbind, lapply(iBF2_lists_0.2null_regrouped, function(sublist) {
        ...
        iBF2_lists_0.2null_regrouped_deltap <- vector("list", num_outerlists)
        # Fill the list of deltas, p values, and MABFs
        for(i in 1:num_outerlists) {
            iBF2_lists_0.2null_regrouped_deltap[[i]] <- list() # Initialize each trial as an empty list
            for(j in 1:num_innermatrixs) {
                # Dynamically create matrix values
                matrix_BFs_deltap <- cbind(rep(c(0,0.2), each = num_deltai), orig_p_matrix_0.2null[, i], obs_d_matrix_0.2null[, i], iBF2_lists_0.2null_regrouped[[i]][[j]])
                # Assign the created matrix to the correct position in iBF2_lists_0.2null_regrouped_deltap
                iBF2_lists_0.2null_regrouped_deltap[[i]][[j]] <- matrix_BFs_deltap
            }
        }
        # Name the lists and matricies 
        iBF2_lists_0.2null_regrouped_deltap <- as.list(iBF2_lists_0.2null_regrouped_deltap)
        # Naming the main sublists
        names(iBF2_lists_0.2null_regrouped_deltap) <- IDs_MABFlists_002
        # Naming contained matricies
        iBF2_lists_0.2null_regrouped_deltap <- lapply(iBF2_lists_0.2null_regrouped_deltap, function(sublist) {
        ...
        for(i in 1:num_outerlists) {
            for(j in 1:num_innermatrixs) {
                colnames(iBF2_lists_0.2null_regrouped_deltap[[i]][[j]]) <- c("delta","original_p","observed_es",1:rep_times)
            }
        }
        # Apply ratesCalc2 to each sublist, combine the results with rbind, and then combine everything into a dataframe
        iBF2_TFSFrates_0.2null <- do.call(rbind, lapply(iBF2_lists_0.2null_regrouped_deltap, function(sublist) {
            do.call(rbind, lapply(sublist, function(x) {
                ratesCalc2(x, pcutoff, EScutoff, BFcutoff, num_deltai)
            }))
        }))
        # Combine all rates to a table
        iBF2_rates_all_0.2null <- cbind(iBF2_TFPNrates_0.2null,iBF2_TFSFrates_0.2null)
        # Compile reportable table
        params_iBF2_0.2null <- expand.grid(
            delta = "0,0.2",
            rep.number = c(2, 5, 10),
            rep.n = c(40, 100, 400),
            orig.n = c(20, 50, 200),
        ...
        params_iBF2_0.2null <- params_iBF2_0.2null[-c(28:108,136:216),]
        row.names(params_iBF2_0.2null) <- NULL
        # analysis_matrix
        rates_iBF2_0.2null <- cbind(params_iBF2_0.2null, iBF2_rates_all_0.2null)
        ...
        # TPR, FPR, TNR, FNR
        # Apply ratesCalc to each sublist, combine the results with rbind, and then combine everything into a dataframe
        iBF2_TFPNrates_0.5null <- do.call(rbind, lapply(iBF2_lists_0.5null_regrouped, function(sublist) {
            do.call(rbind, lapply(sublist, function(x) {
                ratesCalc(x, BFcutoff, num_deltai)
            }))
        }))
        ...
        num_innermatrixs <- length(num_reps)*length(samplesize_reps)
        iBF2_lists_0.5null_regrouped_deltap <- vector("list", num_outerlists)
        # Fill the list of deltas, p values, and MABFs
        for(i in 1:num_outerlists) {
            iBF2_lists_0.5null_regrouped_deltap[[i]] <- list() # Initialize each trial as an empty list
            for(j in 1:num_innermatrixs) {
                # Dynamically create matrix values
                matrix_BFs_deltap <- cbind(rep(c(0,0.5), each = num_deltai), orig_p_matrix_0.5null[, i], obs_d_matrix_0.5null[, i], iBF2_lists_0.5null_regrouped[[i]][[j]])
                # Assign the created matrix to the correct position in iBF2_lists_0.5null_regrouped_deltap
                iBF2_lists_0.5null_regrouped_deltap[[i]][[j]] <- matrix_BFs_deltap
            }
        }
        # Name the lists and matricies 
        iBF2_lists_0.5null_regrouped_deltap <- as.list(iBF2_lists_0.5null_regrouped_deltap)
        # Naming the main sublists
        names(iBF2_lists_0.5null_regrouped_deltap) <- IDs_MABFlists_005
        # Naming contained matricies
        iBF2_lists_0.5null_regrouped_deltap <- lapply(iBF2_lists_0.5null_regrouped_deltap, function(sublist) {
            names(sublist) <- IDs_MABFsubls
            return(sublist)
        })
        # Name the columns of the matricies
        for(i in 1:num_outerlists) {
            for(j in 1:num_innermatrixs) {
                colnames(iBF2_lists_0.5null_regrouped_deltap[[i]][[j]]) <- c("delta","original_p", "observed_es", 1:rep_times)
            }
        }
        # Apply ratesCalc2 to each sublist, combine the results with rbind, and then combine everything into a dataframe
        iBF2_TFSFrates_0.5null <- do.call(rbind, lapply(iBF2_lists_0.5null_regrouped_deltap, function(sublist) {
            do.call(rbind, lapply(sublist, function(x) {
                ratesCalc2(x, pcutoff, EScutoff, BFcutoff, num_deltai)
            }))
        }))
        # Combine all rates to a table
        iBF2_rates_all_0.5null <- cbind(iBF2_TFPNrates_0.5null,iBF2_TFSFrates_0.5null)
        # Compile reportable table
        params_iBF2_0.5null <- expand.grid(
            delta = "0,0.5",
            rep.number = c(2, 5, 10),
            rep.n = c(40, 100, 400),
            orig.n = c(20, 50, 200),
        ...
        # Remove unused conditions
        params_iBF2_0.5null <- params_iBF2_0.5null[-c(28:108,136:216),]
        row.names(params_iBF2_0.5null) <- NULL
        # analysis_matrix
        rates_iBF2_0.5null <- cbind(params_iBF2_0.5null, iBF2_rates_all_0.5null)
        ...
        # Save results
        saveRDS(rates_iBF2_0.2null, paste0(dir_path, "/rates_iBF2_0.2null_c", criterion - 1, ".RDS"))
        saveRDS(rates_iBF2_0.5null, paste0(dir_path, "/rates_iBF2_0.5null_c", criterion - 1, ".RDS"))
    }
}
...
#       iBF2_lists_0.2null_regrouped_deltap[[outerlist_index]] <- list()
#       for (innermatrix_index in 1:num_innermatrixs) {
#         matrix_BFs_deltap <- cbind(rep(c(0,0.2), each = num_deltai), orig_p_matrix_0.2null[, outerlist_index], obs_d_matrix_0.2null[, outerlist_index], iBF2_lists_0.2null_regrouped[[outerlist_index]][[innermatrix_index]])
#         iBF2_lists_0.2null_regrouped_deltap[[outerlist_index]][[innermatrix_index]] <- matrix_BFs_deltap
#       }
#     }
#     
#     iBF2_lists_0.2null_regrouped_deltap <- as.list(iBF2_lists_0.2null_regrouped_deltap)
...
#     for (outerlist_index in 1:num_outerlists) {
#       for (innermatrix_index in 1:num_innermatrixs) {
#         colnames(iBF2_lists_0.2null_regrouped_deltap[[outerlist_index]][[innermatrix_index]]) <- c("delta","original_p","observed_es",1:rep_times)
#       }
#     }
#     
#     iBF2_TFSFrates_0.2null <- do.call(rbind, lapply(iBF2_lists_0.2null_regrouped_deltap, function(sublist) {
...
#     
#     for (outerlist_index in 1:num_outerlists) {
#       iBF2_lists_0.5null_regrouped_deltap[[outerlist_index]] <- list()
#       for (innermatrix_index in 1:num_innermatrixs) {
#         matrix_BFs_deltap <- cbind(rep(c(0,0.5), each = num_deltai), orig_p_matrix_0.5null[, outerlist_index], obs_d_matrix_0.5null[, outerlist_index], iBF2_lists_0.5null_regrouped[[outerlist_index]][[innermatrix_index]])
#         iBF2_lists_0.5null_regrouped_deltap[[outerlist_index]][[innermatrix_index]] <- matrix_BFs_deltap
#       }
#     }
#     
#     iBF2_lists_0.5null_regrouped_deltap <- as.list(iBF2_lists_0.5null_regrouped_deltap)
...
#     for (outerlist_index in 1:num_outerlists) {
#       for (innermatrix_index in 1:num_innermatrixs) {
#         colnames(iBF2_lists_0.5null_regrouped_deltap[[outerlist_index]][[innermatrix_index]]) <- c("delta","original_p", "observed_es", 1:rep_times)
#       }
#     }
#     
#     iBF2_TFSFrates_0.5null <- do.call(rbind, lapply(iBF2_lists_0.5null_regrouped_deltap, function(sublist) {
...
#     
#     # Step 3: Save analysis outcomes
#     saveRDS(rates_iBF2_0.2null, paste0("./MABFanalyses/matrix-wise/rates4Plot/fixed original cutoff/", directory, folder, "/rates_iBF2_0.2null_", folder, ".RDS"))
#     saveRDS(rates_iBF2_0.5null, paste0("./MABFanalyses/matrix-wise/rates4Plot/fixed original cutoff/", directory, folder, "/rates_iBF2_0.5null_", folder, ".RDS"))
#   }
