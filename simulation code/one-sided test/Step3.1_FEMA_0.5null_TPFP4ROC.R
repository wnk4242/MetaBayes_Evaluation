...
FEMA_lists_0.5null_regrouped <- readRDS("./RDS/MABF4ROC/4TPFP/FEMA_lists_0.5null_regrouped.RDS")
...
FEMA_TPFP4ROC_0.5null <- data.frame()

# Set up parallel processing plan
plan(multisession, workers = parallel::detectCores())

# Apply ratesCalcMA_ROC and thresholds to every sublist of MABF values
list_of_sublists <- FEMA_lists_0.5null_regrouped

# Process all sublists in parallel
FEMA_TPFP4ROC_0.5null <- future_map_dfr(names(list_of_sublists), function(sublist_name) {
...
FEMA_TPFP4ROC_0.5null <- FEMA_TPFP4ROC_0.5null %>% 
...
saveRDS(FEMA_TPFP4ROC_0.5null, "./RDS/rates4ROC/FEMA_TPFP4ROC_0.5null.RDS")
