#+++++++++++++++++++++++++++++++++++++++++++++#
#Bayes Factor based on Meta-Analysis (medmed) #
#+++++++++++++++++++++++++++++++++++++++++++++#
######################################################
...
for(j in seq_along(current_rep_results)) {
  ...
  MA_lists_medmed[[j]] <- MA_list  # Store the temporary list in the main list
  names(MA_lists_medmed[[j]]) <- current_IDs[[j]]
}

# Second calculate Bayes factors based on meta-analytic effect sizes
# Find the total number of iterations for the inner loop across the single sublist works.
total_iterations <- length(MA_lists_medmed[[1]])
# Report progress every 10%
progress_threshold <- 0.1*total_iterations
percentage <- 10
increment <- ceiling(0.1 * total_iterations) # Increment to increase the progress threshold
# Initialize an empty list to store results
BFbMA_lists_medmed <- list()
# Nested loops to calculate and store results
for(j in seq_along(MA_lists_medmed)) {
  # Initialize a temporary list to store results for this iteration
  BFbMA_list <- list()
  # Variable to track progress based on k
  count_k <- 0
  for(k in seq_along(MA_lists_medmed[[j]])) {
    BFbMA_templist <- BFbMAcalc(MA_lists_medmed[[j]][[k]])
  ...
  BFbMA_lists_medmed[[j]] <- BFbMA_list  # Store the temporary list in the main list
  names(BFbMA_lists_medmed[[j]]) <- current_IDs[[j]]
}
# Reset to sequential plan
plan(sequential)
names(BFbMA_lists_medmed) <- names(current_IDs)
# Save results
saveRDS(BFbMA_lists_medmed, sprintf("./RDS/MABFoutcomes/500500/BFbMA/medmed/BFbMA_lists_medmed_part%d.RDS", segment_index))
