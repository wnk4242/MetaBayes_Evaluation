#++++++++++++++++++++++++++++++++++++++++++++++#
#Fixed-Effect Meta-Analysis Bayes Factor (hihi)#
#++++++++++++++++++++++++++++++++++++++++++++++#
######################################################
...
for(j in seq_along(current_rep_results)) {
  ...
  FEMABF_lists_hihi[[j]] <- FEMABF_list  # Store the temporary list in the main list
  names(FEMABF_lists_hihi[[j]]) <- current_IDs[[j]]
}
# Reset to sequential plan
plan(sequential)
names(FEMABF_lists_hihi) <- names(current_IDs)
# Save results
saveRDS(FEMABF_lists_hihi, sprintf("./RDS/MABFoutcomes/500500/FEMABF/hihi/FEMABF_lists_hihi_part%d.RDS", segment_index))
