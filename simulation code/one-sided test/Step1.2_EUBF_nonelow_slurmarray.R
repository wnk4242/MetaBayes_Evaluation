...
#++++++++++++++++++++++++++++++++++++++++#
#Evidence Updating Bayes Factor (nonelow)#
#++++++++++++++++++++++++++++++++++++++++#
######################################################
...
for(j in seq_along(current_rep_results)) {
  ...
  EUBF_lists_nonelow[[j]] <- EUBF_list  # Store the temporary list in the main list
  names(EUBF_lists_nonelow[[j]]) <- current_IDs[[j]]
}
# Reset to sequential plan
plan(sequential)
names(EUBF_lists_nonelow) <- names(current_IDs)
# Save results
saveRDS(EUBF_lists_nonelow, sprintf("./RDS/MABFoutcomes/500500/EUBF/nonelow/EUBF_lists_nonelow_part%d.RDS", segment_index))
