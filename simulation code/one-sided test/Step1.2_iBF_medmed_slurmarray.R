# If you use the default prior, this code will take too long to process, so use Step1.2.1_iBF, which is a much faster version (< 3hs to complete)
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
#Inclusion Bayes Factor of Bayesian Averaged Meta-Analysis (medmed)#
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
######################################################
...
# priorEStesting <- prior("cauchy", c(location = 0, scale = 0.707), lower = 0) # this prior is essentially the same as above, but somehow doesnt work
# prior for true effect size (informed, Gronau et al., 2017)
...
for(j in seq_along(current_rep_results)) {
  ...
  iBF_lists_medmed [[j]] <- iBF_list  # Store the temporary list in the main list
  names(iBF_lists_medmed [[j]]) <- current_IDs[[j]]
}
# Reset to sequential plan
plan(sequential)
names(iBF_lists_medmed) <- names(current_IDs)
# Save results
saveRDS(iBF_lists_medmed, sprintf(".//RDS/MABFoutcomes/500500/iBF/medmed/iBF_lists_medmed_part%d.RDS", segment_index))
