...
#####This is a heatmap version of proportions of AD, TP, FP, TN, FN
#####
# library(ggplot2)
# library(dplyr)
# library(tidyr)
...
# # === UI ===
# ui <- fluidPage(
#   titlePanel("MABF Combined Heatmap Explorer"),
#   sidebarLayout(
#     sidebarPanel(
#       selectInput(
#         "bf_cutoff",
...
#         selected = "c1"
#       ),
#       checkboxInput("show_values", "Show values inside tiles", value = TRUE),
#       br(),
#       downloadButton("download_plot", "Download Plot (PNG)")
#     ),
#     mainPanel(
#       plotOutput("heatmapPlot", height = "1200px")
#     )
#   )
# )
# 
...
#     df_null <- df_all %>%
#       filter(true_es == 0.2) %>%
#       select(method, rep.n, rep.number, ADNE, TN, FP) %>%
#       pivot_longer(
#         cols = c(ADNE, TN, FP),
#         names_to = "metric",
#         values_to = "value"
...
#       mutate(
#         theta_num = "0",
#         metric = recode(metric, ADNE = "AD", TN = "TN", FP = "FP")
#       )
#     
#     # theta = 0.2
#     df_02 <- df_all %>%
#       filter(true_es == 0.2) %>%
#       select(method, rep.n, rep.number, ADTE, TP, FN) %>%
#       pivot_longer(
#         cols = c(ADTE, TP, FN),
#         names_to = "metric",
#         values_to = "value"
...
#       mutate(
#         theta_num = "0.2",
#         metric = recode(metric, ADTE = "AD", TP = "TP", FN = "FN")
#       )
#     
#     # theta = 0.5
#     df_05 <- df_all %>%
...
#       group_by(theta_num, method, rep.n, rep.number, metric) %>%
#       summarise(value = sum(value), .groups = "drop") %>%
#       group_by(theta_num, method, rep.n, rep.number) %>%
#       mutate(proportion = value / sum(value)) %>%
#       ungroup() %>%
#       mutate(
#         design = factor(
#           paste0(as.character(rep.number), "_", as.character(rep.n)),
#           levels = c(
#             "10_400", "10_100", "10_40",
#             "5_400", "5_100", "5_40",
#             "2_400", "2_100", "2_40"
...
#   })
#   
#   make_block_plot <- function(df_block, theta_label, metric_levels, show_values = TRUE, show_x = TRUE) {
#     design_labels <- c(
#       "10_400" = "Nrep = 10\nnrep = 400",
#       "10_100" = "Nrep = 10\nnrep = 100",
#       "10_40"  = "Nrep = 10\nnrep = 40",
...
#     
#     p <- ggplot(df_block, aes(x = method, y = design, fill = proportion)) +
...
#       ) +
#       labs(
#         title = bquote(theta == .(theta_label)),
#         x = if (show_x) "MABF Method" else NULL,
#         y = "design"
#       ) +
...
#       p <- p + geom_text(aes(label = sprintf("%.2f", proportion)), size = 3)
#     }
#     
#     p
#   }
...
#     plot_data <- filtered_data()
#     
#     df_0  <- plot_data %>% filter(theta_num == "0")
#     df_02 <- plot_data %>% filter(theta_num == "0.2")
#     df_05 <- plot_data %>% filter(theta_num == "0.5")
#     
#     p0 <- make_block_plot(
#       df_block = df_0,
#       theta_label = "0",
#       metric_levels = c("AD", "TN", "FP"),
...
#     )
#     
#     (p0 / p02 / p05) + plot_layout(guides = "collect") &
...
#     filename = function() {
#       cutoff_lbl <- ifelse(input$bf_cutoff == "c1", "3", "10")
#       paste0("CombinedHeatmap_1sided_BF", cutoff_lbl, ".png")
#     },
#     content = function(file) {
#       ggsave(
#         file,
...
server <- function(input, output) {
  ...
  output$download_plot <- downloadHandler(
    filename = function() {
      cutoff_lbl <- ifelse(input$bf_cutoff == "c1", "3", "10")
      paste0("classic_stackedbar_BFcutoff", cutoff_lbl, "_1sided", ".png")
    },
    content = function(file) {
      ggsave(
        filename = file,
