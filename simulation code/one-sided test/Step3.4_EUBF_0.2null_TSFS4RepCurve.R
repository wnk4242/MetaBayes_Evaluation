...
options(future.globals.maxSize= 26214400000) #To avoid memory limit issues in the future package:25000MB*1024^2

# Read in needed RDS files
EUBF_lists_0.2null_regrouped_deltap <- readRDS("./RDS/MABF4ROC/4TSFS/EUBF_lists_0.2null_regrouped_deltap.RDS")
...
pcutoff <- c(0.001, 0.01, 0.05)
EScutoff <- c(0, 0.1, 0.15)

#Create an empty dataframe to contain data
EUBF_TSFS4RepCurve_0.2null <- data.frame()
...
list_of_sublists <- EUBF_lists_0.2null_regrouped_deltap

# Process all sublists in parallel
EUBF_TSFS4RepCurve_0.2null <- future_map_dfr(names(list_of_sublists), function(sublist_name) {
...
EUBF_TSFS4RepCurve_0.2null <- EUBF_TSFS4RepCurve_0.2null %>% 
...
saveRDS(EUBF_TSFS4RepCurve_0.2null, "./RDS/rates4RepCurve/EUBF_TSFS4RepCurve_0.2null.RDS")
