#++++++++++++++++++++++++++++++++++++++++++++++++#
#Fixed-Effect Meta-Analysis Bayes Factor (medmed)#
#++++++++++++++++++++++++++++++++++++++++++++++++#
######################################################
...
for(j in seq_along(current_rep_results)) {
  ...
  FEMABF_lists_medmed[[j]] <- FEMABF_list  # Store the temporary list in the main list
  names(FEMABF_lists_medmed[[j]]) <- current_IDs[[j]]
}
# Reset to sequential plan
plan(sequential)
names(FEMABF_lists_medmed) <- names(current_IDs)
# Save results
saveRDS(FEMABF_lists_medmed, sprintf("./RDS/MABFoutcomes/500500/FEMABF/medmed/FEMABF_lists_medmed_part%d.RDS", segment_index))
