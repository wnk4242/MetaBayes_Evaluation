...
# put MABF_rates_x_rowise_BFcutoffx.RDS files inside 'for step 0 (raw)' folder
# folder_path <- "./MABFanalyses/row-wise/orig.alpha=0.01/for step 0 (raw)"
# output_path <- file.path(folder_path, "for step 00 (ready to combine)")
...
# take the RDS files inside both 'for step 00 (ready to combine)' folders out to the 'row-wise' folder
# library(dplyr)
# library(purrr)
# library(stringr)
# folder_path <- "./MABFanalyses/row-wise"
# save_path <- file.path(folder_path, "new")  # New folder to save combined files
