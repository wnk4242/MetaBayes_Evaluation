# Bayesian Averaged Meta-Analysis functions
# All functions come from informedTest_functions.R in Gronau et al. (2017)
...
run_meta_bma <- function(params) {
  priorEStesting <- prior(family = "custom",
                          function(x) dst(x, mu = params$mu, sigma = params$sigma, nu = params$nu) /
                            pst(0, mu = params$mu, sigma = params$sigma, nu = params$nu, lower.tail = FALSE),
  ...
  meta_bma(y = d$d, SE = d$se, d = priorEStesting, tau = priorTau, summarize = "integrate")
}
