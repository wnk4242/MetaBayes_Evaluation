...
######################################
#MABF method: FEMABF; dataset: 0.5null#
######################################
...
FEMABF_lists_0.5null_regrouped_deltap <- readRDS("./RDS/MABF4ROC/4TSFS/FEMABF_lists_0.5null_regrouped_deltap.RDS")

# Set up parameters
num_deltai = 500
rep_times = 500
BFcutoff_null <- seq(from = 0.001, by = 0.001, length.out = 999)
BFcutoff_alt <- seq(from = 1.001, by = 0.001, length.out = 1000)
...
FEMABF_TFPS4Viz_0.5null <- data.frame()
...
list_of_sublists <- FEMABF_lists_0.5null_regrouped_deltap

# Process all sublists in parallel
FEMABF_TFPS4Viz_0.5null <- future_map_dfr(names(list_of_sublists), function(sublist_name) {
...
FEMABF_TFPS4Viz_0.5null <- FEMABF_TFPS4Viz_0.5null %>% 
...
saveRDS(FEMABF_TFPS4Viz_0.5null, "./RDS/MABFanalyses/matrix-wise/rates4Viz/FEMABF_TFPS4Viz_0.5null.RDS")
