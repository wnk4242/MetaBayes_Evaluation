#++++++++++++++++++++++++++++++++++++++#
#Evidence Updating Bayes Factor (hihi) #
#++++++++++++++++++++++++++++++++++++++#
######################################################
...
for(j in seq_along(current_rep_results)) {
  ...
  EUBF_lists_hihi[[j]] <- EUBF_list  # Store the temporary list in the main list
  names(EUBF_lists_hihi[[j]]) <- current_IDs[[j]]
}
# Reset to sequential plan
plan(sequential)
names(EUBF_lists_hihi) <- names(current_IDs)
# Save results
saveRDS(EUBF_lists_hihi, sprintf("./RDS/MABFoutcomes/500500/EUBF/hihi/EUBF_lists_hihi_part%d.RDS", segment_index))
