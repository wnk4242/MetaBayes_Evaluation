...
dataset_names <- c("MABF_rates_null_rowise_pcutoff0.05_BFcutoff1",
                   "MABF_rates_null_rowise_pcutoff0.05_BFcutoff3",
                   "MABF_rates_null_rowise_pcutoff0.05_BFcutoff10",
                   "MABF_rates_null_rowise_pcutoff0.05_BFcutoff30")
# Custom labels for metrics
metric_labels <- c("AD_null" = "Anecdotal Evidence", "FP" = "False Positive", "TN" = "True Negative")
# Save the plot as JPG
plot_dir <- "./plots/stacked bar plot/FPTNAD"
...
for (dataset_name in dataset_names) {
  ...
  plot <- ggplot(plot_data_null, aes(x = factor(method), y = proportion, fill = metric)) +
    ...
    labs(x = "Replication Sample Size", y = "Proportion", fill = "Metric", title = paste("Proportions of anecdotal evidence cases, false positives, and true negatives when the cutoff for anecdotal evidence cases is", BFcutoff_value)) +
    facet_grid(rep.n ~ rep.number, scales = "free_x", labeller = label_value) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = "bottom")
  ggsave(filename = paste0("null_stacked_FPTNAD_rowise_pcutoff0.05_BFcutoff", BFcutoff_value, ".jpg"), plot = plot, device = "jpg", width = 14, height = 10, bg = "white")
}
...
original_directory <- getwd()
# List of dataset names
dataset_names <- c("MABF_rates_0.2_rowise_pcutoff0.05_BFcutoff1",
                   "MABF_rates_0.2_rowise_pcutoff0.05_BFcutoff3",
                   "MABF_rates_0.2_rowise_pcutoff0.05_BFcutoff10",
                   "MABF_rates_0.2_rowise_pcutoff0.05_BFcutoff30")
# Custom labels for metrics
metric_labels <- c("AD_true" = "Anecdotal Evidence", "TP" = "True Positive", "FN" = "False Negative")
# Save the plot as JPG
plot_dir <- "./plots/stacked bar plot/TPFNAD"
if (!dir.exists(plot_dir)) {
  dir.create(plot_dir, recursive = TRUE)
}
setwd(plot_dir)
...
plot <- ggplot(plot_data_0.2, aes(x = factor(method), y = proportion, fill = metric)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("AD_true" = "red", "TP" = "green", "FN" = "blue"),
                    labels = as_labeller(metric_labels)) +  # Custom labels using as_labeller()
  labs(x = "Replication Sample Size", y = "Proportion", fill = "Metric", title = paste("Proportions of anecdotal evidence cases, true positives, and false negatives when the cutoff for anecdotal evidence cases is", BFcutoff_value)) +
  facet_grid(rep.n ~ rep.number, scales = "free_x", labeller = label_value) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = "bottom")
  ggsave(filename = paste0("0.2_stacked_TPFNAD_rowise_pcutoff0.05_BFcutoff", BFcutoff_value, ".jpg"), plot = plot, device = "jpg", width = 14, height = 10, bg = "white")
}
# Reset working directory to the original directory
setwd(original_directory)
...
original_directory <- getwd()
# List of dataset names
dataset_names <- c("MABF_rates_0.5_rowise_pcutoff0.05_BFcutoff1",
                   "MABF_rates_0.5_rowise_pcutoff0.05_BFcutoff3",
                   "MABF_rates_0.5_rowise_pcutoff0.05_BFcutoff10",
                   "MABF_rates_0.5_rowise_pcutoff0.05_BFcutoff30")
# Custom labels for metrics
metric_labels <- c("AD_true" = "Anecdotal Evidence", "TP" = "True Positive", "FN" = "False Negative")
# Save the plot as JPG
plot_dir <- "./plots/stacked bar plot/TPFNAD"
...
for (dataset_name in dataset_names) {
  ...
  plot <- ggplot(plot_data_0.5, aes(x = factor(method), y = proportion, fill = metric)) +
    ...
    facet_grid(rep.n ~ rep.number, scales = "free_x", labeller = label_value) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = "bottom")
  ggsave(filename = paste0("0.5_stacked_TPFNAD_rowise_pcutoff0.05_BFcutoff", BFcutoff_value, ".jpg"), plot = plot, device = "jpg", width = 14, height = 10, bg = "white")
}
# Reset working directory to the original directory
setwd(original_directory)
...
############################################################################################################
# Load the data
BiasFilter_null <- MABF_rates_null_rowise_pcutoff0.05_BFcutoff3 %>%
  mutate(orig.n = factor(orig.n, levels = c(20, 50, 200))) %>%
...
# Save the plot
ggsave("./plots/stacked bar plot/filtered stacked bar plot/null_stackedADTSFS_BiasFilter_pcutoff=0.05_BFcutoff=3.jpg", plot = stackedplot_BiasFilter_null, width = 15, height = 10)
rm(list = ls())
...
###########################################################################################################
# Load the data
BiasFilter_0.2 <- MABF_rates_0.2_rowise_pcutoff0.05_BFcutoff3 %>%
  mutate(orig.n = factor(orig.n, levels = c(20, 50, 200))) %>%
  mutate(rep.number = factor(rep.number, levels = c(2, 5, 10), labels = c("N[rep] == 2", "N[rep] == 5", "N[rep] == 10"))) %>%
  mutate(rep.n = factor(rep.n, levels = c(40, 100, 400), labels = c("n[rep] == 40", "n[rep] == 100", "n[rep] == 400"))) %>%
  mutate(bias.level = factor(QRP.level, levels = c("none", "medium", "high"), labels = c("bias = low", "bias = medium", "bias = high"))) %>%
...
# Save the plot
ggsave("./plots/stacked bar plot/filtered stacked bar plot/stackedADTSFS_0.2_BiasFilter_pcutoff=0.05_BFcutoff=3.jpg", plot = stackedplot_BiasFilter_0.2, width = 15, height = 10)
rm(list = ls())
...
###########################################################################################################
# Load the data
BiasFilter_0.5 <- MABF_rates_0.5_rowise_pcutoff0.05_BFcutoff3 %>%
  mutate(orig.n = factor(orig.n, levels = c(20, 50, 200))) %>%
  mutate(rep.number = factor(rep.number, levels = c(2, 5, 10), labels = c("N[rep] == 2", "N[rep] == 5", "N[rep] == 10"))) %>%
  mutate(rep.n = factor(rep.n, levels = c(40, 100, 400), labels = c("n[rep] == 40", "n[rep] == 100", "n[rep] == 400"))) %>%
  mutate(bias.level = factor(QRP.level, levels = c("none", "medium", "high"), labels = c("bias = low", "bias = medium", "bias = high"))) %>%
...
# Save the plot
ggsave("./plots/stacked bar plot/filtered stacked bar plot/stackedADTSFS_0.5_BiasFilter_pcutoff=0.05_BFcutoff=3.jpg", plot = stackedplot_BiasFilter_0.5, width = 15, height = 10)
rm(list = ls())
