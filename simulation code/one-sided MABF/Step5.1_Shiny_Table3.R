# This Shiny app generates the AUC table for underlying effect = 0.2 and 0.5 (one-sided test)
library(shiny)
library(dplyr)
library(tidyr)
library(knitr)
...
server <- function(input, output, session) {
  ...
  output$latex_table <- renderText({
    ...
    kable(
      ...
      col.names = col_names,
      caption = paste0("Area under the curve (AUC), standard deviation (SD), and 95\\% confidence intervals (CI) across MABF methods (one-sided test) by replication sample size ($n_{\\text{rep}}$) and number of replications ($N_{\\text{rep}}$), for true effect size $\\theta = ", input$theta, "$."),
      escape = FALSE
    ) %>%
