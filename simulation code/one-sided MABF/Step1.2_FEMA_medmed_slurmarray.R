#++++++++++++++++++++++++++++++++++++#
#Fixed-Effect Meta-Analysis (medmed) #
#++++++++++++++++++++++++++++++++++++#
######################################################
...
MA_beta_lists_medmed <- list()
MA_p_lists_medmed <- list()
...
for(j in seq_along(current_rep_results)) {
  ...
  MA_beta_lists_medmed[[j]] <- MA_beta_list  # Store the temporary list in the main list
  MA_p_lists_medmed[[j]] <- MA_p_list
  names(MA_beta_lists_medmed[[j]]) <- current_IDs[[j]]
  names(MA_p_lists_medmed[[j]]) <- current_IDs[[j]]
}
# Reset to sequential plan
plan(sequential)
names(MA_beta_lists_medmed) <- names(current_IDs)
names(MA_p_lists_medmed) <- names(current_IDs)
# Save results
saveRDS(MA_beta_lists_medmed, sprintf("./RDS/MABFoutcomes/500500/MA/medmed/FEMA_beta_lists_medmed_part%d.RDS", segment_index))
saveRDS(MA_p_lists_medmed, sprintf("./RDS/MABFoutcomes/500500/MA/medmed/FEMA_p_lists_medmed_part%d.RDS", segment_index))
