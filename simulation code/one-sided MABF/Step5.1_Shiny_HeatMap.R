  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("metric", "Choose Metric", choices = c(
        ...
        "Successful Correction Rate I" = "SCR1",
        "Successful Correction Rate II" = "SCR2"
      )),
...
server <- function(input, output, session) {
  ...
  output$pcutoff_ui <- renderUI({
    if (input$metric %in% c("FSR2", "TSR1", "SCR1", "SCR2")) {
  ...
  output$heatmapPlot <- renderPlot({
    ...
    if (metric == "SCR1") {
      if (collapsed) {
        return(SCR1_heatmap_collapsed(method_name = method, cutoff = cutoff, pcutoff = pcutoff, label_type = label_type, effect_size = effect_size))
      } else {
        return(SCR1_heatmap(method_name = method, cutoff = cutoff, pcutoff = pcutoff, label_type = label_type, effect_size = effect_size))
      }
    }
    if (metric == "SCR2") {
      if (collapsed) {
        return(SCR2_heatmap_collapsed(method_name = method, cutoff = cutoff, pcutoff = pcutoff, label_type = label_type, effect_size = effect_size))
      } else {
        return(SCR2_heatmap(method_name = method, cutoff = cutoff, pcutoff = pcutoff, label_type = label_type, effect_size = effect_size))
      }
    }
  })
}
