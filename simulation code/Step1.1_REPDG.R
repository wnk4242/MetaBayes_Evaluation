# This R script generates replication study results.
# High-performance computers are required to run this R script.
# This R script only generates replications of original studies that are not affected by p-hacking or publication bias.
######################################################
...
plan(sequential)
# Save results
saveRDS(nonelow_rep_results, sprintf("nonelow_rep_results_part%d.RDS", segment_index))
names(nonelow_IDs) <- names(orig_nonelow_lists)
saveRDS(nonelow_IDs, sprintf("nonelow_IDs_part%d.RDS", segment_index))
